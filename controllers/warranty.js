...
const getWarrantyById = async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      return res
        .status(404)
        .json({ message: "No warranty found with the specified ID." });
    }
...
const getWarrantyByProductId = async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      return res
        .status(404)
        .json({ message: "No warranty found for the specified product." });
    }
...
const getWarrantyBySaleId = async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      return res
        .status(404)
        .json({ message: "No warranty found for the specified sale." });
    }
...
const addWarranty = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Warranty added successfully.", result });
  } catch (err) {
...
const updateWarranty = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Warranty updated successfully.", result });
  } catch (err) {
...
const getWarrantiesForReceipt = async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      return res
        .status(404)
        .json({ message: "No warranties found for the specified sale." });
    }
