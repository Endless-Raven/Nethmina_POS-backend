...
const getCashiersByStoreName = async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      return res
        .status(404)
        .json({ message: "No cashiers found for the specified store." });
    }
...
const addCashier = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Cashier added successfully.", result });
  } catch (err) {
...
const updateCashier = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Cashier updated successfully.", result });
  } catch (err) {
...
const getCashiersByStoreId = async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      return res
        .status(404)
        .json({ message: "No cashiers found for the given store." });
    }
    ...
    return res
      .status(500)
      .json({ message: "Error inside server during fetching cashiers.", err });
  }
};
