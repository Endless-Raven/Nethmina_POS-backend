...
const getMonthlyReport = async (req, res) => {
  ...
  if (isNaN(yearInt) || isNaN(monthInt)) {
    return res
      .status(400)
      .json({ message: "Invalid month or year parameter." });
  }
  ...
  try {
    ...
    const totalIncome = formattedReport.reduce(
      (sum, store) => sum + store.sales.reduce((s, sale) => s + sale.income, 0),
      0
    );
    const totalExpense = formattedReport.reduce(
      (sum, store) =>
        sum + store.sales.reduce((s, sale) => s + sale.expence, 0),
      0
    );
    const summary = {
    ...
    return res
      .status(500)
      .json({ message: "Error fetching monthly report", err });
  }
};
...
const getDailyReport = async (req, res) => {
  ...
  try {
    ...
    const reportData = incomeResults
      .concat(expenseResults)
      .sort(
        (a, b) =>
